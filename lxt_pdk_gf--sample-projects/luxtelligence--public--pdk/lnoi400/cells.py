...
def S_bend_vert(
    ...
    bend_ref.move(bend_ref.ports["o1"].dcenter, (0.0, 0.0))
...
def directional_coupler_balanced(
    ...
    c_tl.dmirror_x()
    c_tl.move(c_tl.ports["o1"].dcenter, straight_center_up.ports["o1"].dcenter)

    # bottom left branch
    c_bl = dc << bend_S_spline_varying_width(**settings_s_bend)
    c_bl.dmirror_x()
    c_bl.dmirror_y()
    c_bl.move(c_bl.ports["o1"].dcenter, straight_center_down.ports["o1"].dcenter)
...
def trail_cpw(
    ...
    inc_t2.movey(gap_width_corrected - th)
    inc_t3 = trail_uc << trailround
    inc_t3.movey(-signal_width - th)
    inc_t4 = trail_uc << trailround
    inc_t4.movey(-signal_width - gap_width_corrected)

    # Place T-rails symmetrically w/r to bondpads

    dl_tr = 0.5 * (length - num_cells * tl - (num_cells - 1) * tc)

    [ref.movex(dl_tr) for ref in (inc_t1, inc_t2, inc_t3, inc_t4)]
...
def mzm_unbalanced(
    ...
    **kwargs,
    ...
    rf_line.move(rf_line.ports["e1"].dcenter, (0.0, 0.0))
...
def chip_frame(
    ...
    if center:
        ib.move(origin=(0.0, 0.0), destination=center)
        ob.move(origin=(0.0, 0.0), destination=center)

    x0, y0 = size[0] / 2, size[1] / 2
    dy = size[1] / (num_inputs + 1)
    wgl = 100
    wg = _straight(length=wgl, cross_section=cross_section)
    for i in range(1, num_inputs + 1):
        c.add_ref(wg).move((-x0, -y0 + i * dy))
        c.add_ref(wg).move((x0 - wgl, -y0 + i * dy))

    c.flatten()

    for i in range(1, num_inputs + 1):
        c.add_port(
            name=f"o{i}", center=(-x0 + wgl, -y0 + i * dy), cross_section=cross_section
        )
        c.add_port(
            name=f"o{num_inputs + num_outputs + 1 - i}",
            center=(x0 - wgl, -y0 + i * dy),
            orientation=180,
            cross_section=cross_section,
        )
