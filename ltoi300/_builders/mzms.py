...
def build_terminated_mzm_folded(
    ...
    def _build_layout(comp_len: float):
        c = gf.Component()
        ...
        if dc_phase_shifter_node:
            ps = c << _EO_Phase_shifter(
                ...
                dc_gap=rf_gap,
                dc_central_conductor_width=rf_central_conductor_width,
                dc_pad_width=dc_pad_width,
                dc_ground_width=dc_ground_width,
                compensation_length=comp_len,
        ...
        return c, routes, ps, pad_mod, cpw_mod, term_ref, mmi_in, mmi_out, v_offset_1, h_extent_1, v_offset_2, h_extent_2, v_offset_3, h_extent_3

    # ==========================================================================
    # Two-Pass Path Compensation Solver
    # ==========================================================================
    # To route the folded MZM with a perfect 0.0 um path difference:
    # 1. The detour loop lengths inside the Phase Shifters depend on `compensation_length`.
    # 2. But `compensation_length` depends on the asymmetry between U-turn routes (`r_bottom_len - r_top_len`).
    # 3. And we cannot generate those U-turn routes until the Phase Shifters are instantiated.
    #
    # We solve this circular dependency via a clean two-step layout build flow:
    if compensation_length is None:
        # Step 1 (Pass 1): Build a temporary cell dry-run with zero compensation (comp_len = 0.0).
        # This determines the exact physical ports and routes the U-turn to measure asymmetry.
        _, temp_routes, _, _, _, _, _, _, _, _, _, _, _, _ = _build_layout(comp_len=0.0)
        temp_r_top_len = float(temp_routes[1].length) * 0.001 if (vertical_offset > 0.0 and temp_routes is not None) else 0.0
        temp_r_bottom_len = float(temp_routes[0].length) * 0.001 if (vertical_offset > 0.0 and temp_routes is not None) else 0.0
        
        # Calculate the exact routing length difference (outer route minus inner route)
        routing_diff = float(-temp_r_top_len + temp_r_bottom_len)
        
        # Add 40.0 um for EO phase shifters to offset the asymmetric baseline of detour loops
        compensation_length = routing_diff + (40.0 if dc_phase_shifter_node else 0.0)

    # Step 2 (Pass 2): Discard the temporary dry-run cell, and compile the final layout 
    # using the computed compensation_length. Since the detours preserve output port coordinates,
    # the second pass ports align perfectly with the first pass measurements.
    c, routes, ps, pad_mod, cpw_mod, term_ref, mmi_in, mmi_out, v_offset_1, h_extent_1, v_offset_2, h_extent_2, v_offset_3, h_extent_3 = _build_layout(comp_len=compensation_length)
    ...
    c.info["u_turn_difference"] = float(-r_top_len + r_bottom_len)
